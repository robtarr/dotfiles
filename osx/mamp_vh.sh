...
if [ "$1" = "create" ] || [ "$1" = "add" ]; then
 # Ask for document root
  echo -e "${RED}Enter the document root (relative to '~/'):${NC}";
  read documentRoot;
 
  # Ask for domain name
  echo -e "${RED}Enter local domain: (eg. local.dev):${NC}";
  read domain;
 
   # Ask for port number
  echo -e "${RED}Enter MAMP Port Number:${NC}";
  read port;
  ...
  echo "<VirtualHost *:$port>
    DocumentRoot "/Users/rob/$documentRoot"
    ServerName $domain
    <Directory "/Users/rob/$documentRoot">
        Options All
        AllowOverride All
        Order allow,deny
        Allow from all
